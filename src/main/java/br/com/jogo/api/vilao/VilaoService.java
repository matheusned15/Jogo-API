...
public class VilaoService {
    ...
    public ResponseEntity<Vilao> buscaVilao(Long codigo) {
        ...
        return buscaVilao.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.noContent().build());
    }

    public Vilao buscaVilaoAleatorio() {
        long qntVilaos = vilaoRepository.count();
        Random random = new Random();
        int randomVilao = random.nextInt((int) qntVilaos) + 1;
    ...
    public ResponseEntity<Vilao> atualizaVilao(Long codigo, @Valid Vilao vilao) {
        ...
        if (buscaVilao.isPresent()) {
            ...
            Vilao salva = vilaoRepository.save(buscaVilao.get());
